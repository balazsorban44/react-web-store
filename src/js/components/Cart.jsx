    ...
    if (Object.keys(c).length != 0) {
      return (
        <div id='cart-wrapper'>
          ...
          <div id='cart-items'>
            <h2>{lang.no.cartItems}</h2>
               <CartItems
                 shoppingCart={this.props.shoppingCart}
                 changeQuantity={this.props.changeQuantity}
                 deleteCartItem={this.props.deleteCartItem}
               />
            <div id='cart-sum'>{`${lang.no.cartSum}: ${sum} ${lang.no.currency}`}</div>
            <button id='cart-finish'>{lang.no.finishAndPurchase}</button>
          </div>
        </div>
    )}
          ...
          >
            <div className='cart-item-details'>
              ...
              <div className='cart-item-price'>
                {`${shoppingCart[key].price} ${lang.no.currency}`}
              </div>
