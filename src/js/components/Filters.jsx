...
class SearchFilter extends React.Component {
  ...
  render() {
    return (
      <input id='search-bar' onChange={this.handleEvent} placeholder={lang.no.searchPlaceholder}/>
    )
  }
}
  ...
  render() {
    return (
      <div id='category-filter'>
        <h3>{lang.no.filterByCategory}</h3>
        <ul>
...
class PriceFilter extends React.Component {
  ...
  render() {
    return (
      <div id='price-filter'>
        <h3>{lang.no.filterByPrice}</h3>
        <div>
          <span>{`0 ${lang.no.currency}`}</span>
          <input
          ...
          />
          <span>{`${this.state.price === undefined ? this.state.maxPrice : this.state.price} ${lang.no.currency}`}</span>
        </div>
      </div>
    )
  }
    ...
    return (
      <div id='order'>
        ...
        <select onChange={this.handleEvent} name='price-order'>
          <option value={0}>{lang.no.priceIncrease}</option>
          <option value={1}>{lang.no.priceDecrease}</option>
        </select>
